...
void main() {
  ...
  float p10 = texture(prgm1Texture, uv - e.zy).x;
  float p01 = texture(prgm1Texture, uv - e.xz).x;
  float p21 = texture(prgm1Texture, uv + e.xz).x;
  float p12 = texture(prgm1Texture, uv + e.zy).x;
  vec3 grad = normalize(vec3(p21 - p01, p12 - p10, 1.0));

  grad *= 0.2;

  vec4 c = texture(prgm0Texture, uv + grad.xy);
