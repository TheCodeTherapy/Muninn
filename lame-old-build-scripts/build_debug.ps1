...
.DESCRIPTION
  Builds a debuggable standalone version of the game similar to release build
  but with debugging symbols included. No hot reloading or separate game library.
.PARAMETER Run
...
.EXAMPLE
  .\build_debug.ps1 -Run
  Build and start the debug version
.EXAMPLE
...
function Initialize-BuildDirectories {
  ...
  Write-ColorOutput "Cleaning debug build directory..." $Colors.Info
...
function Build-DebugGame {
  Write-ColorOutput "Building debug executable..." $Colors.Info
  ...
  try {
    ...
    Write-ColorOutput "✅ Debug executable built successfully!" $Colors.Success
    return $true
  }
  catch {
...
function Main {
  Write-ColorOutput "🐛 Odin Debug Build Script" $Colors.Info
  Write-ColorOutput "=========================" $Colors.Info

  # Platform detection (for consistency with hot reload script)
  ...
  # Handle run mode
  if ($Run) {
    Write-ColorOutput "Starting debug game..." $Colors.Info
    try {
      Start-Process -FilePath "$OUT_DIR\$EXE" -WorkingDirectory $OUT_DIR
      Write-ColorOutput "🎮 Debug game started!" $Colors.Success
    }
    catch {
  ...
  Write-ColorOutput "🎉 Debug build complete!" $Colors.Success
  Write-ColorOutput "Debug build created in $OUT_DIR" $Colors.Info
}
